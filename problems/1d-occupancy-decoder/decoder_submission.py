...
def layer_norm(
    batch,
    num_queries,
    block_M=64,  # Reduce to fit dynamic shared memory limit (< 200KB)
    threads=256
):
    width = 768
    ...
    def main(
        ...
        with T.Kernel(T.ceildiv(num_queries, block_M), batch, threads=threads) as (bx, bz):
            ...
            # Use fragment (register) for mean and variance per row
            ...
            # Compute mean: accumulate directly from Input_shared
            T.fill(mean_val, 0)
            for i in T.Parallel(block_M):
                for j in T.serial(width):
                    mean_val[i] += T.float32(Input_shared[i, j])
            for i in T.Parallel(block_M):
                mean_val[i] /= width
            
            # Compute variance: accumulate squared differences
            T.fill(var_val, 0)
            for i in T.Parallel(block_M):
                for j in T.serial(width):
                    diff = T.float32(Input_shared[i, j]) - mean_val[i]
                    var_val[i] += diff * diff
            for i in T.Parallel(block_M):
                var_val[i] = 1.0 / T.sqrt(var_val[i] / width + eps)
...
def output_projection(
    batch,
    num_queries,
    block_M=64,  # Match layer_norm for consistency
    threads=256
):
    width = 768
    out_features = 1
    ...
    def main(
        ...
        with T.Kernel(T.ceildiv(num_queries, block_M), batch, threads=threads) as (bx, bz):
            ...
            # Use fragment array + manual accumulation (same pattern as LayerNorm)
            result_buf = T.alloc_fragment([block_M], accum_dtype)
            
            # Manual dot product: result[i] = sum(input[i, k] * weight[0, k])
            T.fill(result_buf, 0)
            for i in T.Parallel(block_M):
                for k in T.serial(width):
                    result_buf[i] += T.float32(Input_shared[i, k]) * T.float32(Weight[0, k])
            
            # Add bias and write to shared memory
            for i in T.Parallel(block_M):
                Output_shared[i, 0] = T.cast(result_buf[i] + T.float32(Bias[0]), dtype)
            
            # Write to global memory with boundary check
            for i in T.Parallel(block_M):
                q_idx = bx * block_M + i
                if q_idx < num_queries:
                    Output[bz, q_idx, 0] = Output_shared[i, 0]
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # FlashAttention (optimized config for H100: num_stages=1 avoids register pressure)
    ...
    # Step 3: LayerNorm (using TileLang with manual accumulation)
    ln_kernel = layer_norm(batch_size, num_queries, block_M=64, threads=256)
    # TileLang kernel with out_idx auto-allocates output, only pass inputs
    normalized = ln_kernel(attn_output)
    
    # Step 4: Output Projection (using TileLang with manual accumulation)
    out_proj_kernel = output_projection(batch_size, num_queries, block_M=64, threads=256)
    # TileLang kernel with out_idx auto-allocates output, only pass inputs
    result = out_proj_kernel(normalized, weights['out_proj_weight'], weights['out_proj_bias'])
    
    # Copy result to pre-allocated buffer to save allocation time
    out.copy_(result)
